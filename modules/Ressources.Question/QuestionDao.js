...
class QuestionDao extends InterfaceDao {
  ...
  INSERT(question, callback) {
    ...
    const values = [
      questionId,
      getSyntaxe(question.question_title),
      getSyntaxe(question.answer_title_a),
      getSyntaxe(question.answer_title_b),
      getSyntaxe(question.answer_title_c),
      getSyntaxe(question.answer_title_d),
      question.right_answer,
      question.difficulty,
      question.online_exam_ids,
    ];
    const qtext = `INSERT INTO question_table(question_id, question_title, answer_title_a, answer_title_b, answer_title_c, answer_title_d, right_answer, difficulty, online_exam_ids) VALUES (${this.dv(
      values[0]
    )}, ${this.dv(values[1])}, ${this.dv(values[2])}, ${this.dv(
      values[3]
    )}, ${this.dv(values[4])}, ${this.dv(values[5])}, ${this.dv(
      values[6]
    )}, ${this.dv(values[7])}, ${this.dv(values[8])})`;
    console.log(qtext);
    pool
      .query(qtext)
      .then((res) => {
  ...
  UPDATE(question, callback) {
    ...
    const qtext = `UPDATE question_table SET question_title = ${this.dv(
      values[1]
    )}, answer_title_a = ${this.dv(values[2])}, answer_title_b = ${this.dv(
      values[3]
    )}, answer_title_c = ${this.dv(values[4])}, answer_title_d = ${this.dv(
      values[5]
    )}, right_answer = ${this.dv(values[6])}, difficulty = ${this.dv(
      values[7]
    )}, online_exam_ids = ${this.dv(values[8])} WHERE question_id = ${this.dv(
      values[0]
    )}`;
    console.log(qtext);
    pool
  ...
  UPDATE_verified(question, callback) {
    ...
    const qtext = `UPDATE question_table SET verified = ${this.dv(
      values[1]
    )} WHERE question_title = ${this.dv(values[0])}`;
    console.log(qtext);
    pool
      .query(qtext)
      .then((res) => {
  ...
  SELECT(criteria, callback) {
    ...
    if (criteria.online_exam_ids !== undefined)
      qtext += ` WHERE online_exam_ids @> ARRAY[${criteria.online_exam_ids}]::int[]`;
    if (criteria.question_ids)
      qtext = this.actq(qtext, "question_id", criteria.question_ids);
    if (criteria.order) qtext = this.aotq(qtext, criteria.order, criteria.by);
    if (criteria.limit) qtext = this.altq(qtext, criteria.limit);
    console.log(qtext);
    pool
      .query(qtext)
      .then((res) => {
